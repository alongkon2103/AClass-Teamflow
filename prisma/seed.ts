...
async function main() {
  ...
  const t1 = await prisma.task.create({
    data: {
      ...
      description: "วางเลย์เอาต์ด่าน 4-6 พร้อมกับดักและศัตรู (งานร่วมกันสองคน)",
      status: TaskStatus.DOING,
      priority: Priority.IMPORTANT,
      startDate: day(-5),
      dueDate: day(2),
      sortOrder: 0,
      // Shared task: two people collaborate on the same card.
      assignees: { create: [{ userId: napa.id }, { userId: thana.id }] },
  ...
  await prisma.task.create({
    data: {
      ...
      dueDate: day(-1),
      sortOrder: 0,
      assignees: { create: [{ userId: thana.id }] },
      createdById: leader.id,
  ...
  const t3 = await prisma.task.create({
    data: {
      ...
      dueDate: day(7),
      sortOrder: 1,
      assignees: { create: [{ userId: ploy.id }] },
      createdById: leader.id,
  ...
  const t4 = await prisma.task.create({
    data: {
      ...
      dueDate: day(1),
      sortOrder: 0,
      assignees: { create: [{ userId: napa.id }] },
      createdById: leader.id,
      gameId: pixelRacer.id,
    },
  });
