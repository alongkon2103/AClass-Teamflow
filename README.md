...
งานที่ขึ้นสถานะ **เสร็จสิ้นแล้ว** จะอยู่บนบอร์ดอีก 3 วันแล้วย้ายเข้าแท็บ **คลังงาน** เอง
การ์ดบนบอร์ดจะนับถอยหลังให้เห็นก่อน (`เข้าคลังในอีก 2 วัน`) จะได้ไม่หายไปแบบไม่รู้ตัว

การย้ายไม่ได้ตั้งเวลาไว้ด้วย cron แต่ทำตอนเริ่มใช้งานแอป (ทั้งเว็บและ CLI) แล้วข้ามตัวเองถ้าทำไปแล้ววันนี้
— ไม่มี scheduler ให้ต้องดูแล และไม่มีทางเพี้ยนถ้ามันหยุดทำงาน

งานเก่าที่ไม่มีวันที่เสร็จบันทึกไว้จะใช้วันที่แก้ไขล่าสุดแทน เฉพาะเพื่อจัดบอร์ดให้สะอาดเท่านั้น
— การตัดสินว่าส่งตรงเวลาหรือล่าช้ายังไม่เดาให้เหมือนเดิม

ในคลังค้นหาและกรองตามคนหรือเกมได้ และกด **นำกลับมาทำต่อ** เพื่อดึงกลับขึ้นบอร์ด
งานที่เสร็จแล้วจะกลับมาในสถานะ _กำลังทำ_ พร้อมล้างวันที่เสร็จ ไม่งั้นรอบถัดไปมันจะถูกเก็บเข้าคลังทันที
...
พิมพ์ `teamflow` เฉย ๆ จะเปิดบอร์ดที่เลื่อนด้วยลูกศร ไม่ต้องจำ id หรือ flag

| ปุ่ม         | ทำอะไร                    |
| ------------ | ------------------------- |
| `↑` `↓`      | เลื่อนงาน (`k` `j` ก็ได้) |
| `Home` `End` | ไปบนสุด / ล่างสุด         |
| `Enter`      | เปิดดูรายละเอียด          |
| `m`          | ย้ายสถานะ (เลือกจากเมนู)  |
| `l`          | บันทึกความคืบหน้า         |
| `d`          | สลับแสดงงานที่เสร็จแล้ว   |
| `q` `Esc`    | ออก                       |

เมนูย่อยใช้ปุ่มชุดเดียวกัน — `Space` ติ๊กเลือกในรายการที่เลือกได้หลายอัน `a` เลือกทั้งหมด
และถ้าอ้างชื่องานแล้วตรงหลายงาน จะขึ้นเมนูให้เลือกแทนที่จะฟ้อง error
...
ผลลัพธ์เป็นตารางมีกรอบ ปรับความกว้างตาม terminal เอง (บังคับได้ด้วย `COLUMNS=140`)
และปิดสีด้วย `NO_COLOR=1` ได้

```
╭──────────┬──────────┬────────┬──────────────────────┬──────────┬──────────────────────────┬───────╮
│ ID       │ สถานะ    │ ระดับ   │ ชื่องาน                │ ผู้รับผิดชอบ │ กำหนดส่ง                  │ อัปเดต │
├──────────┼──────────┼────────┼──────────────────────┼──────────┼──────────────────────────┼───────┤
│ cmtayjxf │ ● ต้องทำ  │ ▲ สำคัญ │ Monthly Payment Sum… │ Mashe    │ 3 ก.ย. 2569 (อีก 5 วัน)   │     0 │
│ cmtd08wz │ ● กำลังทำ │ ▲ สำคัญ │ Competitor Research… │ Mashe    │ 31 ส.ค. 2569 (อีก 2 วัน)  │     1 │
╰──────────┴──────────┴────────┴──────────────────────┴──────────┴──────────────────────────┴───────╯
  5 งาน · ต้องทำ 3  ·  กำลังทำ 2
```
